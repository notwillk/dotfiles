...
RULE_FILE_MATCHER="../rules/*.rule.sh"
...
unload_file(){ unset ID DESCRIPTION SKIP_VALIDATE_AFTER_FIX applies check can_fix fix; }
load_file(){
  unload_file
...
verbose_log "ℹ️  Rules: $RULE_FILE_MATCHER"
verbose_log "ℹ️  Command: $CMD"
if [[ -n "$TARGET_ID" ]]; then
  verbose_log "ℹ️  Rule ID: $TARGET_ID"
else
  verbose_log "ℹ️  Rule ID: *"
fi
...
for file in "${RULE_FILES[@]}"; do
  ...
  verbose_log "📄 Rule file: $file"
  verbose_log "📣  Rule ID: $ID"
  verbose_log "💡  Rule description: $DESCRIPTION"

  if [[ -n "$TARGET_ID" && "$TARGET_ID" != "$ID" ]]; then
    verbose_log "🫥  Skipping rule $ID"
    continue
  fi
  ...
  case "$CMD" in
    list) echo "📋 $ID - $DESCRIPTION" ;;
    check) check "$VERBOSE" && echo "✅ $ID" || { echo "❌ $ID"; RC=1; } ;;
    fix)
      if check "$VERBOSE"; then
        ...
        if has can_fix && ! can_fix "$VERBOSE"; then
          ...
          if has SKIP_VALIDATE_AFTER_FIX && [[ "$SKIP_VALIDATE_AFTER_FIX" = "true" ]]; then
            echo "⚠️  $ID - skipping validation after fix"
          else
            if check "$VERBOSE"; then
              echo "🛠️ $ID - fixed!"
              FIXED=$((FIXED + 1))
...
if (( FIXED > 0 )); then
  echo "⚠️  Done, you may need to restart your shell for changes to take effect"
fi
